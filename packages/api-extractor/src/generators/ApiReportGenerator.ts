...
function capitalizeFirstLetter(input: string): string {
	return input[0] ? `${input[0].toLocaleUpperCase()}${input.slice(1)}` : '';
}
...
export class ApiReportGenerator {
	...
	private static _shouldIncludeInReport(
		...
		const releaseTag: ReleaseTag =
			apiItemMetadata.effectiveReleaseTag === ReleaseTag.None ? ReleaseTag.Public : apiItemMetadata.effectiveReleaseTag;
